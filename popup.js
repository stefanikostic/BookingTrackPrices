  ...
  chrome.storage.local.get([SELECTED_ACCOMMODATIONS_KEY], (result) => {
    ...
    if (selectedAccommodations) {
      for (let i = 0; i < selectedAccommodations.length; i++) {
        ...
        bookingSearchResultsUrlObj.searchParams.set(SEARCH_STRING, selectedAccommodation.bookingPropertyName);
        bookingSearchResultsUrlObj.searchParams.set(CHECK_IN, selectedAccommodation.startDate);
        bookingSearchResultsUrlObj.searchParams.set(CHECK_OUT, selectedAccommodation.endDate);
        bookingSearchResultsUrlObj.searchParams.set(NUMBER_OF_ADULTS, selectedAccommodation.numberOfAdults ? selectedAccommodation.numberOfAdults : DEFAULT_NUMBER_OF_ADULTS);
        bookingSearchResultsUrlObj.searchParams.set(NUMBER_OF_ROOMS, selectedAccommodation.numberOfRooms ? selectedAccommodation.numberOfRooms : DEFAULT_NUMBER_OF_ROOMS);
        bookingSearchResultsUrlObj.searchParams.set(NUMBER_OF_CHILDREN, selectedAccommodation.numberOfChildren ? selectedAccommodation.numberOfChildren : DEFAULT_NUMBER_OF_CHILDREN);
        ...
        fetch(bookingSearchResultsUrlObj.href) 
          ...
          .then((data) => {
            ...
            const propertyCard = doc.querySelector('[data-testid="property-card"]');

            let oldPrice = selectedAccommodation.price;
            let updatedPrice = extractPrice(propertyCard);
            let priceElement = "";
            if (updatedPrice === null) {
              priceElement = `<div class="not-available-property align-self-end mt-auto mb-2 mr-2">
            ...
            document.getElementById(BOOKINGS_ELEMENT_ID).innerHTML += `<li>
            ...
            <div class="row">
              ...
              <div class="col-5 my-2 p-0">
                <h6 class="card-title display-6">${selectedAccommodation.bookingPropertyName}</h6>
                <p class="card-subtitle room-type">${selectedAccommodation.roomType}</p>
                <p class="card-text dates">Start Date: ${selectedAccommodation.startDate}</p>
                <p class="card-text dates">End Date: ${selectedAccommodation.endDate}</p>
              </div>` + 
...
function extractPrice(propertyCard) {
  let priceElement = propertyCard.querySelector('[data-testid="price-and-discounted-price"]');
